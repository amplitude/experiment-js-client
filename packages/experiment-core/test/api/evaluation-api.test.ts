...
describe('Evaluation API', () => {
  it('should get variants', async () => {
    ...
    const evaluationApi = new SdkEvaluationApi('test-deployment-key', 'https://server.url.amplitude.com', mockHttpClient);
  ...
  it('should get variants with options', async () => {
    ...
    expect(variants).toEqual(VARIANTS);
    expect(mockHttpClient.request).toHaveBeenCalledWith({
      requestUrl: 'https://server.url.amplitude.com/sdk/v2/vardata?v=0&eval_mode=local&delivery_method=web',
      method: 'GET',
      headers: {
        Authorization: 'Api-Key test-deployment-key',
        'X-Amp-Exp-User': Base64.encodeURL(JSON.stringify(USER)),
