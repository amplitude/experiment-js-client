...
describe('initializeExperiment', () => {
  ...
  describe('consent-aware exposure handling', () => {
    let mockExposureTrackingProvider: any;

    beforeEach(() => {
      mockExposureTrackingProvider = {
        track: jest.fn(),
      };
    });
    ...
    it('should store exposures in memory when consent is PENDING', () => {
      ...
      const client = DefaultWebExperimentClient.getInstance(
        ...
        JSON.stringify(DEFAULT_PAGE_OBJECTS),
        {
          exposureTrackingProvider: mockExposureTrackingProvider,
        },
      );

      client.start();

      // Exposure should be called but not tracked to the provider yet
      expect(mockExposure).toHaveBeenCalledWith('test');
    ...
    it('should track exposures immediately when consent is GRANTED', () => {
      ...
      // Exposure should be called and tracked to the provider immediately
      expect(mockExposure).toHaveBeenCalledWith('test');
    ...
    it('should not track exposures when consent is REJECTED', () => {
      ...
      // Exposure should be called but not tracked to the provider
      expect(mockExposure).toHaveBeenCalledWith('test');
      expect(mockExposureTrackingProvider.track).not.toHaveBeenCalled();
    });
    ...
    it('should fire all pending exposures when consent changes from PENDING to GRANTED', () => {
      ...
      const client = DefaultWebExperimentClient.getInstance(
        stringify(apiKey),
        JSON.stringify([
          createMutateFlag('test-flag-1', 'treatment', [DEFAULT_MUTATE_SCOPE]),
          createMutateFlag('test-flag-2', 'control', [DEFAULT_MUTATE_SCOPE]),
        ]),
        JSON.stringify({
          'test-flag-1': createPageObject(
            'A',
            'url_change',
            undefined,
            'http://test.com',
          ),
          'test-flag-2': createPageObject(
            'A',
            'url_change',
            undefined,
            'http://test.com',
